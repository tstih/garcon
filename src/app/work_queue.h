...
public:
    ...
    // requested.
    [[nodiscard]] queue_push_result push(net::socket socket, std::stop_token stop = {});

    // Attempts to enqueue immediately without blocking.
    [[nodiscard]] queue_push_result try_push(net::socket socket);
    ...
    [[nodiscard]] std::optional<net::socket> pop(std::stop_token stop = {});
